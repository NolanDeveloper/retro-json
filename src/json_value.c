...
extern void json_value_free_internal(struct jsonValue * value) {
    if (!value) return;
    switch (value->kind) {
    case JVK_STR:  json_string_free_internal(&value->v.string); return;
    case JVK_NUM:  return;
    case JVK_OBJ:  json_object_free_internal(&value->v.object); return;
    case JVK_ARR:  json_array_free_internal(&value->v.array);   return;
